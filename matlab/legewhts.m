function [ts,whts] = legewhts(n)%rts(n)
...
x0 = 0.0;
% [pol,der] = lege.pol(x0,n); %%%%%changehere
[pol,der] = legepol(x0,n); %%%%%changehere
...
for kk = 1:length(tsr)
    ...
    for i=1:nnewt
        ...
        h=x1-x0;
%         [pol,der] = lege.tayl(polpre,derpre,x0,h,n,k);%%%%change here
        [pol,der] = legetayl(polpre,derpre,x0,h,n,k);%%%%change here
        x1=x1-pol/der;
