function [u_bd,u_domain]=calculate_scat_field(NODES,OPERATORS,PARAMETERS,xx,yy,fsource)
% global OPERATORS
% global NODES
% global PARAMETERS
% global xx
...
flag_green=0;
[ugreen] = greens_formula_nh(xtrg,ytrg,u_scat2,C,[0,1,kh],kh,ww,u_homo/2,L*un_part/2,T_new,theta,flag_green);    
u_bd=ugreen(:);
...
%     x_tmp2=xx(2,ind);
    ...
    [n1,n2]=size(fsource);
    out=zeros(n2,n1);            
%     eta=interp2(PARAMETERS.XP,PARAMETERS.YP,Noise,x_tmp1,x_tmp2);    
%     aux = -kh*kh*repmat(eta,n1,1).*fsource;            
    aux=kh*kh*fsource;
