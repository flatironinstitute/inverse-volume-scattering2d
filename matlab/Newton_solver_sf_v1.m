function [ domain, it_newton, rhs_out, iesc, iter_lsqr ] = ...
  Newton_solver_sf_v1(N_Newton_it, eps_dq, eps_res, kh, nmodes, ...
  npoints, ntheta, radius, len2, Np, Ncheb, u_sol, reg_param_vec, ...
  domain_init)
...
while flag_newton
    ...
    dq=zeros(1,PARAMETERS.nmodes*PARAMETERS.nmodes);
     ...
     % solve least squares problem either using lsqr or dense matrix
     % inversion
    if kh<=15
	    ...
	    tic
            [dq_newton, flag, relres, iter] = ...
              run_lsqr_derivative(PARAMETERS, OPERATORS, NODES, xx, ...
              yy, u_scat_domain_newton + transpose(u_inc), 1e-4, ...
              N_it_lsqr, rhs_orig);
	    toc
